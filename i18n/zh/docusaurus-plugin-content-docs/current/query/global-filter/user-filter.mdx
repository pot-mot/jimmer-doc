...
-   `AssociationIntegrityAssuranceFilter/KAssociationIntegrityAssuranceFilter`
...
为此，Jimmer支持<ViewMore buttonText="多视角缓存"><MultiViewCache/></ViewMore>来解决这个问题，但代价是相关的全局过滤器必须实现`CachableFilter`/`KCachableFilter`接口。
...
相比于最基本的`Fitler`/`KFitler`接口而言，此接口并未新加任何方法，仅用作类型标识。
